...
using PositionConstraints =
    std::vector<PositionConstraint, pmr::polymorphic_allocator<PositionConstraint>>;

/// @brief Collection of velocity constraints.
using VelocityConstraints =
    std::vector<VelocityConstraint, pmr::polymorphic_allocator<VelocityConstraint>>;
...
auto At(Container&& container, ::playrho::detail::IndexingNamedType<U, V> id, Message&& msg)
    -> decltype(OutOfRange{id,
                           std::forward<Message>(
                               msg)}, // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
                std::forward<Container>(container).at(to_underlying(id)))
{
    try {
        ...
        std::throw_with_nested(OutOfRange{
            id, std::forward<Message>(
                    msg)}); // NOLINT(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
    }
}

inline void IntegratePositions(const Span<const BodyID>& bodies,
                               const Span<BodyConstraint>& constraints, Time h)
{
    assert(IsValid(h));
...
inline void Report(const ContactImpulsesFunction& listener, const Span<const ContactID>& contacts,
...
BodyConstraints GetBodyConstraints(pmr::memory_resource& resource, const Span<const BodyID>& bodies,
                                   const ObjectPool<Body>& bodyBuffer, Time h,
                                           ...
                                           const VelocityConstraint::Conf conf)
{
    ...
    transform(
        cbegin(contacts), cend(contacts), back_inserter(velConstraints),
        [&](const auto& contactID) {
            ...
            const auto xfA =
                GetTransformation(bodyConstraintA.GetPosition(), bodyConstraintA.GetLocalCenter());
            const auto xfB =
                GetTransformation(bodyConstraintB.GetPosition(), bodyConstraintB.GetLocalCenter());
            const auto& manifold = manifoldBuffer[to_underlying(contactID)];
            return VelocityConstraint{
                friction,     restitution,
                tangentSpeed, GetWorldManifold(manifold, xfA, radiusA, xfB, radiusB),
                bodyA,        bodyB,
                bodies,       conf};
        });
...
/// @details This updates positions (and nothing else) by calling the position constraint solving
/// function.
...
{
    const auto underactive =
        IsUnderActive(GetVelocity(b), conf.linearSleepTolerance, conf.angularSleepTolerance);
    const auto sleepable = IsSleepingAllowed(b);
    return (sleepable && underactive) ? GetUnderActiveTime(b) + conf.deltaTime : 0_s;
}

inline Time UpdateUnderActiveTimes(const Span<const BodyID>& bodies, ObjectPool<Body>& bodyBuffer,
                                   const StepConf& conf)
{
...
inline BodyCounter Sleepem(const Span<const BodyID>& bodies, ObjectPool<Body>& bodyBuffer)
{
                      ...
                      const KeyedContactIDs& contactsBodyB, BodyID bodyB) noexcept
{
    ...
    for (const auto& ci : contactsBodyB) {
        auto& contact = contactBuffer[to_underlying(ci.second)];
...
{
    return std::any_of(begin(contacts), end(contacts),
                       [](const Contact& contact) { return contact.NeedsUpdating(); });
}
...
bool ShouldCollide(const ObjectPool<Joint>& jointBuffer, const ObjectPool<BodyJointIDs>& bodyJoints,
...
void Unset(cista::offset::vector<bool>& islanded, const BodyIDs& elements)
{
...
void Unset(cista::offset::vector<bool>& islanded, const KeyedContactIDs& elements)
{
    for (const auto& [contactKey, contactID] : elements) {
        islanded[to_underlying(contactID)] = false;
    }
}
...
void ResetBodiesForSolveTOI(BodyIDs& bodies, ObjectPool<Body>& buffer) noexcept
{
    for_each(begin(bodies), end(bodies),
             [&](const auto& body) { buffer[to_underlying(body)].ResetAlpha0(); });
}
...
auto CreateProxies(DynamicTree& tree, BodyID bodyID, ShapeID shapeID, const Shape& shape,
                   const Transformation& xfm0, const Transformation& xfm1, const StepConf& conf,
                   ProxyIDs& fixtureProxies, ProxyIDs& otherProxies) -> ChildCounter
{
...
auto Find(const C& c, const V& value) -> decltype(std::find(begin(c), end(c), value), bool{})
{
    const auto last = end(c);
    return std::find(begin(c), last, value) != last;
}
...
void ResizeAndReset(cista::offset::vector<T>& vector,
                    typename cista::offset::vector<T>::size_type newSize, const U& newValue)
{
    std::fill(begin(vector), begin(vector) + ToSigned(std::min(size(vector), newSize)), newValue);
    vector.resize(newSize);
}

/// @brief Removes <em>unspeedables</em> from the is <em>is-in-island</em> state.
BodyIDs::size_type RemoveUnspeedablesFromIslanded(const Span<const BodyID>& bodies,
                                                  const ObjectPool<Body>& buffer,
                                                  cista::offset::vector<bool>& islanded)
{
...
auto FindContacts(pmr::memory_resource& resource, const DynamicTree& tree, const ProxyIDs& proxies)
...
auto Append(cista::offset::vector<std::pair<BodyID, ShapeID>>& fixtures, BodyID bodyId,
            Span<const ShapeID> shapeIds) -> void
{
...
AabbTreeWorld::AabbTreeWorld(const WorldConf& conf)
    : m_statsResource(conf.doStats ? conf.upstream : nullptr),
      m_bodyStackResource(GetBodyStackOpts(conf), conf.doStats ? &m_statsResource : conf.upstream),
...
void AabbTreeWorld::Remove(BodyID id)
{
    ...
    const auto it = std::find(begin(m_bodies), end(m_bodies), id);
    if (it != cend(m_bodies)) {
...
void Destroy(AabbTreeWorld& world, BodyID id)
{
    ...
    while (!joints.empty()) {
        ...
        const auto endIter = end(world.m_joints);
        const auto iter = std::find(begin(world.m_joints), endIter, jointID);
        if (iter != endIter) {
...
void AabbTreeWorld::Remove(JointID id)
{
    ...
    if ((!collideConnected) && (bodyIdA != InvalidBodyID) && (bodyIdB != InvalidBodyID)) {
        if (FlagForFiltering(m_contactBuffer, bodyIdA, m_bodyContacts[to_underlying(bodyIdB)],
                             bodyIdB)) {
            m_flags |= e_needsContactFiltering;
        }
    }
...
void Destroy(AabbTreeWorld& world, JointID id)
{
    ...
    const auto endIter = end(world.m_joints);
    const auto iter = std::find(begin(world.m_joints), endIter, id);
    if (iter != endIter) {
        world.Remove(id);
        world.m_joints.erase(iter);
...
ShapeID CreateShape(AabbTreeWorld& world, Shape def)
{
    ...
    return static_cast<ShapeID>(
        static_cast<ShapeID::underlying_type>(world.m_shapeBuffer.Allocate(std::move(def))));
}
...
void Destroy(AabbTreeWorld& world, ShapeID id)
{
    ...
    for (auto bodyIdx = static_cast<decltype(GetBodyRange(world))>(0); bodyIdx < numBodies;
         ++bodyIdx) {
...
void SetShape(AabbTreeWorld& world, ShapeID id,
              Shape def) // NOLINT(readability-function-cognitive-complexity)
{
    if (IsLocked(world)) {
        throw WrongState(worldIsLockedMsg);
    }
    ...
    for (auto&& b : world.m_bodyBuffer) {
        ...
        if (geometryChanged && IsEnabled(b)) {
            const auto bodyId =
                BodyID(static_cast<BodyID::underlying_type>(&b - data(world.m_bodyBuffer)));
            ...
            Erase(world.m_bodyContacts[to_underlying(bodyId)],
                  [&world, bodyId, id, &b](ContactID contactID) {
...
void AabbTreeWorld::AddToIsland(Island& island, BodyID seedID, BodyCounter& remNumBodies,
...
void AabbTreeWorld::AddToIsland(Island& island, BodyStack& stack, BodyCounter& remNumBodies,
                                ContactCounter& remNumContacts, JointCounter& remNumJoints)
{
    while (!empty(stack)) {
        ...
        // Adds appropriate contacts of current body and appropriate 'other' bodies of those
        // contacts.
...
void AabbTreeWorld::AddJointsToIsland(Island& island, BodyStack& stack, const BodyJointIDs& joints)
{
    for_each(cbegin(joints), cend(joints), [this, &island, &stack](const auto& ji) {
        ...
        if (!m_islanded.joints[to_underlying(jointID)]) {
            ...
            const auto other = (otherID == InvalidBodyID) ? static_cast<Body*>(nullptr)
...
RegStepStats AabbTreeWorld::SolveReg(const StepConf& conf)
{
        ...
        if (!m_islanded.bodies[to_underlying(bodyId)]) {
            ...
            if (IsAwake(body) && IsEnabled(body)) {
                ...
                stats.maxIslandBodies =
                    std::max(stats.maxIslandBodies, static_cast<BodyCounter>(size(island.bodies)));
...
#if defined(DO_THREADED)
                // Updates bodies' sweep.pos0 to current sweep.pos1 and bodies' sweep.pos1 to new
                // positions
                futures.push_back(std::async(
                    std::launch::async, &AabbTreeWorld::SolveRegIslandViaGS, this, conf, island));
#else
    ...
    for (const auto& bodyId : m_bodies) {
        if (m_islanded.bodies[to_underlying(bodyId)]) {
            ...
            if (IsSpeedable(body)) {
                ...
                stats.proxiesMoved +=
                    Synchronize(m_bodyProxies[to_underlying(bodyId)], GetTransform0(GetSweep(body)),
                                GetTransformation(body), conf);
            }
        }
    }
    ...
    stats.contactsAdded = AddContacts(
        FindContacts(m_proxyKeysResource, m_tree, std::exchange(m_proxiesForContacts, {})), conf);
...
IslandStats AabbTreeWorld::SolveRegIslandViaGS(const StepConf& conf, const Island& island)
{
    ...
    auto bodyConstraints = GetBodyConstraints(m_bodyConstraintsResource, island.bodies,
                                              m_bodyBuffer, h, GetMovementConf(conf));
    ...
    auto velConstraints = GetVelocityConstraints(
        m_velocityConstraintsResource, island.contacts, m_contactBuffer, m_manifoldBuffer,
        m_shapeBuffer, bodyConstraints, GetRegVelocityConstraintConf(conf));
    if (conf.doWarmStart) {
    ...
    // updates array of tentative new body positions per the velocities as if there were no
    // obstacles...
    ...
    for (auto i = decltype(conf.regPositionIters){0}; i < conf.regPositionIters; ++i) {
        const auto minSeparation =
            SolvePositionConstraintsViaGS(posConstraints, bodyConstraints, psConf);
...
AabbTreeWorld::UpdateContactsData AabbTreeWorld::UpdateContactTOIs(const StepConf& conf)
{
    auto results = UpdateContactsData{};

    const auto toiConf = GetToiConf(conf);
    for (const auto& contact : m_contacts) {
        auto& c = m_contactBuffer[to_underlying(contact.second)];
        if (HasValidToi(c)) {
            ++results.numValidTOI;
            continue;
        }
        ...
        const auto toi = IsValidForTime(output.state)
                             ? std::min(alpha0 + (Real(1) - alpha0) * output.time, Real(1))
                             : Real(1);
        ...
        for (const auto& bodyId : m_bodies) {
            if (m_islanded.bodies[to_underlying(bodyId)]) {
                ...
                if (IsAccelerable(body)) {
                    stats.proxiesMoved +=
                        Synchronize(m_bodyProxies[to_underlying(bodyId)],
                                    GetTransform0(body.GetSweep()), GetTransformation(body), conf);
...
IslandStats AabbTreeWorld::SolveToi(ContactID contactID, const StepConf& conf)
{
    ...
    const auto toi =
        ZeroToUnderOneFF<Real>(*GetToi(contact)); // NOLINT(bugprone-unchecked-optional-access)
...

    {
        ...
        assert((toi != Real(0)) ||
               ((GetSweep(bA).alpha0 == Real(0)) && (GetSweep(bB).alpha0 == Real(0))));
        Advance(bA, toi);
        ...
        if (!IsEnabled(contact) || !IsTouching(contact)) {
            ...
            SetSweep(bB, backupB);
            return IslandStats{}
                .IncContactsUpdated(numUpdated)
                .IncContactsSkipped(numUpdated ? 0u : 1u);
        }
    }
    if (IsSpeedable(bA)) {
        bA.SetAwakeFlag();
    ...
    return SolveToiViaGS(island, subConf)
        .IncContactsUpdated(numUpdated)
        .IncContactsSkipped(numSkipped);
}
...
IslandStats AabbTreeWorld::SolveToiViaGS(const Island& island, const StepConf& conf)
{
     ...
     */
    auto bodyConstraints = GetBodyConstraints(m_bodyConstraintsResource, island.bodies,
                                              m_bodyBuffer, 0_s, GetMovementConf(conf));
    ...
    auto velConstraints = GetVelocityConstraints(
        m_velocityConstraintsResource, island.contacts, m_contactBuffer, m_manifoldBuffer,
        m_shapeBuffer, bodyConstraints, GetToiVelocityConstraintConf(conf));
...
AabbTreeWorld::ProcessContactsForTOI( // NOLINT(readability-function-cognitive-complexity)
    BodyID id, Island& island, ZeroToUnderOneFF<Real> toi, const StepConf& conf)
{
                ...
                if (bodyImpenetrable || IsImpenetrable(other)) {
                    const auto otherIslanded = m_islanded.bodies[to_underlying(otherId)];
                    {
                        ...
                        if (!otherIslanded /* && GetSweep(other).alpha0 != toi */) {
                            Advance(other, toi);
                            if (GetPosition0(other) != backup.pos0 ||
                                GetPosition1(other) != backup.pos1) {
                                FlagForUpdating(m_contactBuffer,
                                                m_bodyContacts[to_underlying(otherId)]);
                            }
                        }
                        ...
                        // Revert and skip if contact disabled by user or not touching anymore (very
                        // possible).
                        if (!IsEnabled(contact) || !IsTouching(contact)) {
                    ...
                    if (!otherIslanded) {
                        ...
                        /*
                         * If other is-in-island but not in current island, then something's gone
                         * wrong. Other needs to be in current island but was already in the island.
                         * A previous contact island didn't grow to include all the bodies it needed
                         * or perhaps the current contact is-touching while another one wasn't and
                         * the inconsistency is throwing things off.
                         */
...
{
    assert((world.m_vertexRadius.GetMax() * Real(2)) + (conf.linearSlop / Real(4)) >
           (world.m_vertexRadius.GetMax() * Real(2)));
    ...
    auto stepStats = StepStats{};
    {
        ...
        for (const auto& [bodyID, shapeID] : world.m_fixturesForProxies) {
            ...
            stepStats.pre.proxiesCreated += CreateProxies(
                world.m_tree, bodyID, shapeID, world.m_shapeBuffer[to_underlying(shapeID)], xfm0,
                xfm1, conf, world.m_bodyProxies[to_underlying(bodyID)], world.m_proxiesForContacts);
        }
        ...
        stepStats.pre.proxiesMoved = [&world](const StepConf& cfg) {
            ...
            for_each(begin(world.m_bodiesForSync), end(world.m_bodiesForSync),
                     [&world, &cfg, &proxiesMoved](const auto& bodyID) {
                         ...
                         const auto xfm0 = GetTransform0(GetSweep(body));
                         const auto xfm1 = GetTransformation(body);
                         proxiesMoved += world.Synchronize(
                             world.m_bodyProxies[to_underlying(bodyID)], xfm0, xfm1, cfg);
                     });
            return proxiesMoved;
        }(conf);
        ...
        // Note: this may update bodies (in addition to the contacts container).
        stepStats.pre.contactsAdded =
            world.AddContacts(FindContacts(world.m_proxyKeysResource, world.m_tree,
...
AabbTreeWorld::UpdateContactsStats AabbTreeWorld::UpdateContacts(const StepConf& conf)
{
    ...
    return UpdateContactsStats{static_cast<ContactCounter>(updated),
                               static_cast<ContactCounter>(skipped)};
}

ContactCounter AabbTreeWorld::AddContacts( // NOLINT(readability-function-cognitive-complexity)
    std::vector<ProxyKey, pmr::polymorphic_allocator<ProxyKey>>&& keys, const StepConf& conf)
{
    ...
    for_each(cbegin(keys), cend(keys), [this, &updateConf](const ProxyKey& key) {
        ...
        // W/ World::list<Contact> and Body::list<ContactKey,Contact*>
        // .444s@step15, 1.063s-sumstep20 W/ World::list<Contact> and
        // Body::list<ContactKey,Contact*>   .393s@step15, 1.063s-sumstep20 W/ World::list<Contact>
        // and Body::list<ContactKey,Contact*>   .412s@step15, 1.012s-sumstep20 W/
        // World::list<Contact> and Body::vector<ContactKey,Contact*> .219s@step15, 0.659s-sumstep20
        ...
        if (FindTypeValue((size(contactsA) < size(contactsB)) ? contactsA : contactsB,
                          std::get<0>(key))) {
            return;
        }
    ...
    if (numContactsAdded > 0u) {
        sort(begin(m_contacts), end(m_contacts),
             [](const KeyedContactID& a, const KeyedContactID& b) {
...
const cista::offset::vector<DynamicTree::Size>& GetProxies(const AabbTreeWorld& world, BodyID id)
{
...
ContactCounter AabbTreeWorld::Synchronize(const ProxyIDs& bodyProxies, const Transformation& xfm0,
                                          const Transformation& xfm1, const StepConf& conf)
{
    ...
    for (auto&& e : bodyProxies) {
        ...
        const auto aabb = ComputeAABB(
            GetChild(m_shapeBuffer[to_underlying(leafData.shapeId)], leafData.childId), xfm0, xfm1);
        // Note: updating leaf here is expensive, avoid when possible!
        if (!Contains(node.GetAABB(), aabb)) {
            m_tree.UpdateLeaf(
                e, GetDisplacedAABB(GetFattenedAABB(aabb, conf.aabbExtension), displacement));
        ...
        for (auto i = decltype(new_point_count){0}; i < new_point_count; ++i) {
            if (!found[i]) {
                ...
                for (auto j = decltype(old_point_count){0}; j < old_point_count; ++j) {
                    ...
                    const auto squareDiff =
                        GetMagnitudeSquared(oldPt.localPoint - newPt.localPoint);
...
void SetBody(AabbTreeWorld& world, BodyID id, Body value)
{
    ...
    if (!empty(shapeIds.first)) {
        ...
        const auto lastProxy = end(bodyProxies);
        bodyProxies.erase(
            std::remove_if(begin(bodyProxies), lastProxy,
    ...
    for (auto&& shapeId : shapeIds.first) {
        // Destroy any contacts associated with the fixture.
        Erase(world.m_bodyContacts[to_underlying(id)],
              [&world, id, shapeId, &elem](ContactID contactID) {
                            ...
                            const Span<const Contact>& contacts) noexcept
{
    ...
    for (const auto& id : ids) {
        const auto contactID = id.second;
